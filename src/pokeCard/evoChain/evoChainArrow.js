...
function EvoChainArrow(props){
    ...
    return (
		<span className="text-center EvoChainArrow">
            ...
            {EvoDetails && EvoDetails.trigger == "Trade" && !EvoDetails.trade_species && <i title="Trade" className="fas fa-lg d-block fa-people-arrows"></i>}
            {EvoDetails && EvoDetails.trigger == "Trade" && EvoDetails.trade_species && <div><i title={"Trade for " + EvoDetails.trade_species} className="fas fa-lg d-block fa-people-arrows"></i> {EvoDetails.trade_species}</div>}
            {EvoDetails && EvoDetails.item && <div>{EvoDetails.item}</div>}
            {EvoDetails && EvoDetails.held_item && <div>{EvoDetails.held_item}</div>}
            ...
            {EvoDetails && EvoDetails.needs_overworld_rain && <div title="Overworld Rain or Fog"><i class="fas fa-tint"></i></div>}
            {EvoDetails && EvoDetails.gender == "Male" && <div title="Male"><i class="fas fa-lg fa-mars"></i></div>}
            {EvoDetails && EvoDetails.gender == "Female" && <div title="Female"><i class="fas fa-lg fa-venus"></i></div>}
            <br />
		</span>
	);
			...
			if(value){
				switch(key){
                    ...
                    case "held_item":
                        ...
                        break;
                    case "trade_species":
                        deets.trade_species = helpers.capitalize(value.name.replaceAll("-"," "));
                        break;
					default: 
